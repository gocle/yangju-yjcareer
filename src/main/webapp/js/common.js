...
var com = {
		 ...
		 */	
		fileDownRequestUrl : "/comm/atchfle/atchFileDown.do",
		fileDeleteRequestUrl : "/comm/atchfle/atchFileDelete.do",   
		splitChar : "|",  
...
//							message : "<img src='" + CONTEXT_ROOT + "/js/jquery/plugins/blockUI/progress_2_2.gif' />",
							message : "<img src='/image/common/progress_4_5.gif' />",
...
//			}			
		...
		emailcheck : function(strValue) {
			...
			//var regExp = /^([\w]{1,})+[\w\.\-\_]+([\w]{1,})+@(?:[\w\-]{2,}\.)+[a-zA-Z]{2,}$/;
...
//	    	$('<input></input>').attr({type:type, name:inputId, id:inputId, value:value }).appendTo($form);
	  ...
	  datepicker : function(vCalId, vOpt){
		  ...
		  var dateOptions = { 
				    showOn: showOn,
				    buttonImage: '/image/adm/calendar.png', // 버튼 이미지
...
/* 20180801 LMS lu 페이지 신규 팝업 및 페이지구성 관련 js */
function callCustomModal(pop){
	document.getElementById(pop).classList.add("active");
	document.querySelector("body").classList.add("fixed");
};
function closeCustomModal(pop){
	document.getElementById(pop).classList.remove("active");
	document.querySelector("body").classList.remove("fixed");
};
function chkTextAreaRows(event){
	var thisArea = event.target;
	var thisRows = thisArea.value.split(/\r*\n/);
	var thisCount = thisRows.length;
	thisArea.style.height = (26 * parseInt(thisCount)) + "px";
};