...
function fnEnrollUpdate(seqCd, memId, seqno) {
		...
		<table class="list-1" width="100%;" style="margin-top:20px;">
			<thead>
				<tr>
					...
					<th style="width: 12%">휴대전화</th>
					<th style="width: 20%">신청일자</th>
					<th style="width: 20%">신청상태</th>
					<th style="width: 20%">승인일자</th>
					<th style="width: 12%">관리자 등록여부</th>
				</tr>
			</thead>
			<tbody>
				<c:forEach var="item" items="${resultList}" varStatus="status">
					<tr>
						<td><input type="checkbox" name="memIds" class="all_check" value="<c:out value="${item.memId}_${item.seqno}"/>"></td>
						<td><c:out value="${totalCount - ((pageIndex-1) * pageSize + status.index)}"/></td>
						<td><a href="#" onclick="javascript:fnEnrollUpdate('${item.seqCd}', '${item.memId}', '${item.seqno}');">${item.memName }</a></td>
						<td>${item.hpTel1 }</td>
						<td>${item.regDt }</td>
						<td>
							<c:choose>
