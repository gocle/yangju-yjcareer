...
function fn_save(){
	...
	$("#form").attr("action", "${contextRoot}/adm/product/insert.do");
...
function fn_update() {
	...
	$("#productDesc").val(data);
	$("#form").attr("action", "${contextRoot}/adm/product/update.do");
	$("#form").attr("target","_self");
	$("#form").submit();
}
...
function fn_list() {
	...
	$("#form").attr("action", "${contextRoot}/adm/product/list.do?menuId="+menuId);
								...
								<c:forEach var="fileVO1" items="${fileList1}" varStatus="status">
									<c:if test="${fileVO1.thumbnailCrop eq 'Y'}">
										<span>
											...
											<button type="button" onclick="fn_egov_deleteFile('${fileVO1.atchFileIdx}','/adm/product/${boardConfigVO.bcId}/updateForm.do?productId=${productVO.productId}');">삭제</button>
										</span>
									</c:if>
								</c:forEach>
