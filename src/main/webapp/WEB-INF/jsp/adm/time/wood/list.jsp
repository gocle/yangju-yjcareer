...
function fn_detail( pgType , pgId){
	...
	var reqUrl = "${contextRoot}/adm/program/time/"+pgType+"/timeList.do?pgId="+pgId+"&menuId="+menuId;
	$("#form").attr("action", reqUrl);
	$("#form").submit();
} 
	...
	<tbody>
		<c:forEach var="resultList" items="${resultList}" varStatus="status">
			<tr>
				...
				<td>
					<c:choose>
						<c:when test="${resultList.pgTarget eq 'ONEDAY'}">
							원데이클래스
						</c:when>
						<c:when test="${resultList.pgTarget eq 'DIY'}">
							DIY
						</c:when>
						<c:when test="${resultList.pgTarget eq 'CHILD'}">
							유치원
						</c:when>
						<c:otherwise>
							-
