							...
							<c:forEach var="fileVO" items="${fileList}" varStatus="status">
								<c:if test="${fileVO.thumbnailCrop eq 'N'}">
									<span>
										...
										<button type="button" onclick="fn_egov_deleteFile('${fileVO.atchFileIdx}','/adm/bbs/${boardConfigVO.bcId}/updateForm.do?baId=${boardArticleVO.baId}&bcId=${boardArticleVO.bcId}');">삭제</button>
									</span>
								</c:if>
							</c:forEach>
							...
							<c:forEach var="fileVO1" items="${fileList1}" varStatus="status">
								<c:if test="${fileVO1.thumbnailCrop eq 'Y'}">
									<span>
										...
										<button type="button" onclick="fn_egov_deleteFile('${fileVO1.atchFileIdx}','/adm/bbs/${boardConfigVO.bcId}/updateForm.do?baId=${boardArticleVO.baId}&bcId=${boardArticleVO.bcId}');">삭제</button>
									</span>
								</c:if>
							</c:forEach>
