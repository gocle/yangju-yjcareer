...
function fn_search(param1){
	...
	var reqUrl ="${contextRoot}/adm/reservation/"+pgType+"/list.do";
	
	$("#form").attr("action", reqUrl);
	$("#form").submit();
}

function fn_applyList( pgType , pgId, pgCode){
	...
	var reqUrl = "${contextRoot}/adm/reservation/"+pgType+"/applyList.do?pgId="+pgId+"&menuId="+menuId;
	$("#form").attr("action", reqUrl);
	$("#form").submit();
} 
		...
		<ul class="search-box">
			<li>
				...
				<select id="searchCondition" name="searchCondition">
					...
					<option value="PG_NAME" ${reservationVO.searchCondition == 'PG_NAME' ? 'selected="selected"' : ''}>제목</option>
					<option value="PG_DESC" ${reservationVO.searchCondition == 'PG_DESC' ? 'selected="selected"' : ''}>내용</option>
				</select>
				<input type="text" style="width:300px" value="${reservationVO.searchKeyword}" name="searchKeyword" id="searchKeyword" maxlength="10" placeholder="검색어 입력" />
			</li>
			<li class="search-btn">
	...
	<tbody>
		<c:forEach var="resultList" items="${resultList}" varStatus="status">
			<tr>
				...
				<td>
					<a href="javascript:fn_applyList('${resultList.pgType}','${resultList.pgId}','${resultList.pgCode}');">
						${resultList.pgName}
					</a>
				</td>
				<td>
				...
				<td>
					<c:if test = "${resultList.pgCode == 'CHILD_REG'}">
					${resultList.capacity * 2  * resultList.locationCnt} 명
					</c:if>
					<c:if test = "${resultList.pgCode == 'FOREST_INT'}">
					${resultList.capacity * 2} 명
					</c:if>
				</td>
				<td>
					<c:if test = "${resultList.pgCode == 'CHILD_REG'}">
					${resultList.applyCnt} / ${resultList.capacity * 2 * resultList.locationCnt}
					</c:if>
					<c:if test = "${resultList.pgCode == 'FOREST_INT'}">
					${resultList.applyCnt} / ${resultList.capacity * 2}
					</c:if>
				</td>
			</tr>
		</c:forEach>
