...
var locationInfo = [
	  <c:forEach var="loc" items="${locationInfo}" varStatus="s">
		...
		console.log("=== LOCATION & CNT 목록 ===");
		locationInfo.forEach(item => {
		  console.log("location:", item.location, " / cnt:", item.locationCnt);
		});
	  ...
	  var ORIG_STATUS      = '${reservationVO.status}';  // 기존 예약 상태

	  function capacityValidateSimple(){
		var groupYn = $('.groupYn').val();
	    var newPeople = parseInt($('.peopleCnt').val(), 10) || 0;
	    var status = $("select[name='status']").val(); // 예약상태
	    
	    if (((status === 'WAIT' || status === 'CONFIRM') && PG_CODE === 'FOREST_INT')) {
	        var exists = locationInfo.some(r => r.location === CURRENT_LOCATION);

	        if (!exists) {
	            alert("장소는 제한되어 등록할 수 없습니다.");
	            return false;
	        }
	    }
	    ...
	    if(ORIG_STATUS === 'CANCEL' || ORIG_STATUS === 'REJECT'){
	    	ORIG_PEOPLE = 0;
	    }
	    ...
	    // (기존 인원 제외 후 + 새 인원)
	    var adjusted = locCnt - ORIG_PEOPLE + newPeople;
	    var limit = 0;
	    
	    console.log("11111111111111");
	    console.log("row = " + row);
	    console.log("locCnt = " + locCnt);
	    console.log("ORIG_PEOPLE = " + ORIG_PEOPLE);
...
function fn_list() {
	...
	$("#form").attr("action", "${contextRoot}/adm/reservation/"+pgType+"/applyList.do?menuId="+menuId);
