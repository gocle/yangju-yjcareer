...
function fn_search(param1){
	...
	var reqUrl ="${contextRoot}/adm/reservation/"+pgType+"/applyList.do";
...
function fn_detail(resvId, pgType) {
	...
	$("#pgType").val( pgType );
	$("#resvId").val(resvId);
	var reqUrl = "${contextRoot}/adm/reservation/" + pgType + "/detail.do?menuId="+menuId;
...
function fn_list() {
	...
	$("#form").attr("action", "${contextRoot}/adm/reservation/"+pgType+"/list.do?menuId="+menuId);
...
<table class="list-1">
	...
	<tbody>
		<c:forEach var="resultList" items="${resultList}" varStatus="status">
			<tr>
				...
				<td>
					<c:choose>
				        <c:when test="${resultList.attendYn eq 'Y'}">참석</c:when>
				        <c:when test="${resultList.attendYn eq 'N'}">미참석</c:when>
				        <c:otherwise>-</c:otherwise>
				    </c:choose>
				</td>
	...
	<div class="text-right btn-area">
		<a href="javascript:void(0);" id="btn_apply" class="point" data-url="${contextRoot}/adm/reservation/${programVO.pgType}/applyForm.do?menuId=${menuId}&pgId=${programVO.pgId}&pgCode=${pgCode}">
		   참여자 등록
		</a>
