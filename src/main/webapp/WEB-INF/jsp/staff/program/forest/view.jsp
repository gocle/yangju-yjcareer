...
function fn_delete(){
	...
	if(confirm("삭제하시겠습니까?")){
		var reqUrl = "${contextRoot}/staff/program/"+pgType+"/delete.do?&memuId="+menuId;
...
function fn_update(){
	...
	var pgType = '${programVO.pgType}';
	var menuId = "${menuId}";
	var reqUrl = "${contextRoot}/staff/program/"+pgType+"/updateForm.do";
...
function fn_list(){
	...
	var pgType = '${programVO.pgType}';
	var menuId = $("#menuId").val();
	var reqUrl = "${contextRoot}/staff/program/"+pgType+"/list.do?menuId="+menuId
...
function fn_copy() {
	...
	$("#form").attr("action", "${contextRoot}/staff/program/"+pgType+"/copy.do");
	...
	<form id="form" name="form" method="post">
		...
		<table class="detail">
			...
			<tbody>
				...
				<tr>
					<th>프로그램 분류</th>
					<td colspan="5">${programVO.pgCodeNm}</td>
				</tr>
				<tr>
					<th>프로그램 소개</th>
