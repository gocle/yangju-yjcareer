...
function fn_save(){
	...
	$("#pgDesc").val(data);
	$("#form").attr("action", "${contextRoot}/staff/program/"+pgType+"/insert.do");
...
function fn_update() {
	...
	$("#pgDesc").val(data);
	$("#form").attr("action", "${contextRoot}/staff/program/"+pgType+"/update.do");
	$("#form").attr("pgTarget","_self");
	$("#form").submit();
}
...
function fn_list() {
	...
	$("#form").attr("action", "${contextRoot}/staff/program/"+pgType+"/list.do?menuId="+menuId);
								...
								<c:if test="${fileVO.thumbnailCrop eq 'N'}">
									<span id="file${fileVO.atchFileIdx}">
										<a href="javascript:fn_egov_downFile('${fileVO.atchFileIdx}');">
											...
											<button type="button" onclick="fn_egov_deleteFile('${fileVO.atchFileIdx}','/staff/program/${programVO.pgType}/updateForm.do?pgId=${programVO.pgId}', 'attach');">삭제</button>
									</span>
								</c:if>
							</c:forEach>
								...
								<c:forEach var="fileVO1" items="${fileList1}" varStatus="status">
									<c:if test="${fileVO1.thumbnailCrop eq 'Y'}">
										<span id="file${fileVO1.atchFileIdx}">
											...
											<button type="button" onclick="fn_egov_deleteFile('${fileVO1.atchFileIdx}','/staff/program/${programVO.pgType}/updateForm.do?pgId=${programVO.pgId}', 'thumb');">삭제</button>
										</span>
									</c:if>
								</c:forEach>
...
<script>
  ...
  function loadPlaceOptions(group, selectedCsv) {
	  ...
	  $.getJSON('${contextRoot}/staff/program/' + pgType + '/codes.do', { group: group }, function(list){
