...
function fn_editor( brId , baId , index){
	...
	var pathId = $("#bcId").val();
	      ...
	      $.ajax({
	         url:"${contextRoot}/staff/bbs/"+pathId+"/detail.do?menuId="+menuId,
	         type:"post",
	         data:{
	        	 "brId":brId,
	        	 "baId":baId
...
function fn_delete(){
	...
	if(confirm("자료를 삭제하시겠습니까?")){
		var reqUrl = "${contextRoot}/staff/bbs/"+pathId+"/delete.do?memuId="+menuId;
...
function fn_update(){
	...
	$("#form").attr("action", "${contextRoot}/staff/bbs/"+bcId+"/update.do" );
...
function fn_list(){
	...
	var reqUrl = "${contextRoot}/staff/bbs/"+bcId+"/list.do";
...
/* 	var text = data.replace(/[<][^>]*[>]/gi, "");
	...
	$("#replyForm").attr("action", "${contextRoot}/staff/bbs/"+pathBcId+"/replyInsert.do");
							...
							<c:if test="${boardReplyList.rDeleteYn eq 'N'}">
															<tr>
																...
																<td>
								                     					   ...
								                     					   <a href="javascript:fn_egov_replyDeleteFile('${boardReplyList.atchFileIdx}', '/${siteId}/staffin/bbs/${result.bcId}/detail.do?baId=${boardReplyList.baId}&menuId=${menuId}' )" class="btn">파일삭제</a>		   
								                     			</td>
															</tr>
															</c:if>
							-->
