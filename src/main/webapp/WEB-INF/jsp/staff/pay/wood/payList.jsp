...
function fn_search(param1){
	...
	var reqUrl ="${contextRoot}/staff/pay/"+pgType+"/payList.do";
...
function fn_list() {
	...
	$("#form").attr("action", "${contextRoot}/staff/pay/"+pgType+"/list.do?menuId=" + menuId);
	$("#form").submit()
}
...
function fn_update() {
	...
	$("#form").attr("action", "${contextRoot}/staff/pay/"+pgType+"/payForm.do");
	$("#form").submit()
}
</script>
	...
	<tbody>
		<c:forEach var="payList" items="${payList}" varStatus="status">
            <tr>
                ...
                <td>${payList.note}</td>
                <td>
                	<button type="button" onclick="window.open('${contextRoot}/staff/pay/${pgType}/taxPopup.do?payId=${payList.payId}', 'taxPopup', 'width=900,height=700')">출력</button>
                </td>
            </tr>
        </c:forEach>
