...
function fn_search(param1){
	...
	var reqUrl ="${contextRoot}/staff/reservation/"+pgType+"/list.do";
	
	$("#form").attr("action", reqUrl);
	$("#form").submit();
}

function fn_payList( pgType , pgId, pgName){
	...
	var reqUrl = "${contextRoot}/staff/pay/"+pgType+"/payList.do?pgId="+pgId+"&menuId="+menuId;
	$("#form").attr("action", reqUrl);
	$("#form").submit();
} 
...
<section>
	<form id="form" name="form" method="get">
		...
		<input type="hidden" id="pgName" name="pgName" />
...
<table class="list-1">
	<thead>
		<tr>
			...
			<th style="width: 10%">참석인원</th>
			<th style="width: 10%">금액합계</th>
		</tr>
	</thead>
	<tbody>
		<c:forEach var="resultList" items="${resultList}" varStatus="status">
			<tr>
				...
				<td>
					<a href="javascript:fn_payList('${resultList.pgType}','${resultList.pgId}','${resultList.pgName}');">
						${resultList.pgName}
					</a>
				</td>
				<td>
				...
				<td>
					${resultList.attendCnt} 명
				</td>
				<td>
					${resultList.totalAmount}
				</td>
			</tr>
		</c:forEach>
