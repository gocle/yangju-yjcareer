...
function fn_delete(){
	...
	if(confirm("삭제하시겠습니까?")){
		var reqUrl = "${contextRoot}/staff/reservation/"+pgType+"/delete.do?&memuId="+menuId;
...
function fn_update(){
	...
	var pgType = '${reservationVO.pgType}';
	var menuId = "${menuId}";
	var reqUrl = "${contextRoot}/staff/reservation/"+pgType+"/updateForm.do";
...
function fn_applyList(){
	...
	var pgType = '${reservationVO.pgType}';
	var slotDate = '${reservationVO.slotDate}';
	$("#slotDate").val( slotDate ); 
	var menuId = $("#menuId").val();
	var reqUrl = "${contextRoot}/staff/reservation/"+pgType+"/applyList.do?menuId="+menuId
...
<section>
	<form id="form" name="form" method="post">
		...
		<input type="hidden" id="slotId" name="slotId" value="${reservationVO.slotId}" />
		...
		<table class="detail">
			...
			<tbody>
				<tr>
					...
					<td>
						<c:choose>
				            <c:when test="${reservationVO.groupYn eq 'Y'}">
				                남 ${reservationVO.mCnt}명 / 여 ${reservationVO.fCnt}명
				            </c:when>
				            <c:otherwise>
				                <c:choose>
				...
				<tr>
					<th>신청차시</th>
					<td>${reservationVO.slotNo}부 (${reservationVO.startTime} ~ ${reservationVO.endTime})</td>
					<th>이메일</th>
					<td>${reservationVO.email}</td>
					<th>공예품</th>
					<td>
						${reservationVO.productName}
						<c:if test="${not empty reservationVO.optionName}">
							( + ${reservationVO.optionName})
						</c:if>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<c:choose>
				            <c:when test="${reservationVO.groupYn eq 'Y'}">
				                총 ${reservationVO.peopleCnt}명 (남 ${reservationVO.mCnt}, 여 ${reservationVO.fCnt})
				            </c:when>
				            <c:otherwise>
				                ${reservationVO.peopleCnt}
				            </c:otherwise>
				        </c:choose>
					</td>
				</tr>
				...
				<tr rowspan="2">
					...
					<td colspan="5">
					<c:choose>
						<c:when test="${reservationVO.note eq ''}">-</c:when>
						<c:when test="${reservationVO.groupYn ne ''}">${reservationVO.note}</c:when>
					</c:choose>
					</td>
				</tr>
			</tbody>
