...
function fn_detail(resvId, pgType) {
	...
	$("#pgType").val( pgType );
	$("#resvId").val(resvId);
	var reqUrl = "${contextRoot}/staff/reservation/" + pgType + "/detail.do";
...
function updateTotalPeople(row) {
    ...
    row.find('.totalCnt').val(mCnt + fCnt);
}
</script>
			...
			<tr>
				...
				<td>
					...
					<div class="personal-field" style="display: ${reservationVO.groupYn eq 'Y' ? 'none' : 'block'};">
					    <input type="number" name="peopleCnt" value="${reservationVO.peopleCnt}" class="peopleCnt" />
					</div> -->
		        </td>
			</tr>
			<tr>
