...
function fn_detail(pgType, pgId, pgCode){
	$("#pgType").val( pgType );
	$("#pgId").val( pgId ); 
	$("#pgCode").val( pgCode ); 
...
<c:import url="/usr/layout/top.do" />
  ...
  <c:import url="/usr/menu/header.do" />
	...
	<section class="section programS-list search-tab flex center">
		...
		<c:if test="${pgCode ne null && pgCode ne '' && pgType eq 'wood'}">
			<!-- <dl>
				<dt>프로그램</dt>
				<dd>목공체험</dd>
        ...
        <c:forEach var="resultList" items="${resultList}" varStatus="status">
        	<article class="program-item" aria-labelledby="p1-title">
        		<c:if test="${empty resultList.pgThumbpath}">
        			<div class="program-thumb" style="background-image:url('${contextRoot}/assets/css/usr/img/block.png');" role="img" aria-label="산림체험 이미지"></div>
        		</c:if>
        		<c:if test="${not empty resultList.pgThumbpath}">
        			<div class="program-thumb" style="background-image:url('${contextRoot}/thumbnail/${resultList.pgThumbpath}');" role="img" aria-label="산림체험 이미지"></div>
        		</c:if>
	          ...
	          <div class="program-body">
				...
				<div class="program-actions flex center">
					<div>
						...
						<a href="javascript:fn_detail('${resultList.pgType}','${resultList.pgId}','${resultList.pgCode}');" class="p1-title">[${resultList.pgCodeNm}]${resultList.pgName}</a>
					</div>
					<a href="javascript:fn_detail('${resultList.pgType}','${resultList.pgId}','${resultList.pgCode}');"  class="btn" aria-label="6월 성인반 목공체험 자세히 보기">자세히 보기</a>
				</div>
				...
				<div class="programS-txt">
					...
					<dl>
						<dt>신청인원</dt>
						<dd>${resultList.capacity}명</dd>
					</dl>
					<dl>
						<dt>장소</dt>
						<dd>${resultList.locationNm}</dd>
					</dl>
					<dl>
						<dt>비용</dt>
						<dd>5,000원</dd>
