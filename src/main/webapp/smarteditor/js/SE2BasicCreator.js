function createSEditor2(elIRField, htParams, elSeAppContainer){
	...
	//if(htParams.bSkipXssFilter !== true){
		...
		/*var test = '<img src="x" onerror="alert(1)">';  
		alert("test : "+test);
		var filter = /<\/*(?:onerror|onchange)|l(?:applet|b(?:ase|gsound|link)|embed|frame(?:set)?|i(?:frame|layer)|l(?:ayer|ink)|meta|object|s(?:cript|tyle)|title|xml)[^>]*?>/gi
		alert("test replace : "+test.replace(filter, ""));
		var newText = test.replace(/onerror|onchange/gi,"");
